  ...
  Object.entries(tokens.typography).forEach(([element, config]) => {
    if (typeof config === "object" && config !== null) {
      if ("fontFamily" in config) {
        variables[`--${prefix}-font-${element}-family`] = config.fontFamily;
        variables[`--${prefix}-font-${element}-size`] = config.fontSize;
        variables[`--${prefix}-font-${element}-weight`] = config.fontWeight.toString();
        variables[`--${prefix}-font-${element}-line-height`] = config.lineHeight;
        if (config.letterSpacing) {
          variables[`--${prefix}-font-${element}-letter-spacing`] = config.letterSpacing;
        }
      } else {
        Object.entries(config).forEach(([breakpoint, typography]) => {
          if (typeof typography === "object" && "fontFamily" in typography) {
            variables[`--${prefix}-font-${element}-${breakpoint}-family`] = typography.fontFamily;
            variables[`--${prefix}-font-${element}-${breakpoint}-size`] = typography.fontSize;
            variables[`--${prefix}-font-${element}-${breakpoint}-weight`] = typography.fontWeight.toString();
            variables[`--${prefix}-font-${element}-${breakpoint}-line-height`] = typography.lineHeight;
            if (typography.letterSpacing) {
              variables[`--${prefix}-font-${element}-${breakpoint}-letter-spacing`] = typography.letterSpacing;
            }
          }
        });
      }
